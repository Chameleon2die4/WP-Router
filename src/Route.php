...
class Route extends Utility
{
    ...
    public function execute( WP $query ) {
        ...
        new Page($page_contents, $title, $template);
    }
    ...
    protected function get_query_args( WP $query, string $callback_type = 'page' ) {
     ...
     * @return string
     */
    protected function preg_index( int $matches ) {
        global $wp_rewrite;
        $wp_rewrite->matches = 'matches'; // because it may not be set, yet
        return $wp_rewrite->preg_index($matches);
    }
